...
// HOOKS
$plugins->add_hook('admin_rpgstuff_update_stylesheet', 'postinggoal_admin_update_stylesheet');
$plugins->add_hook('admin_rpgstuff_update_plugin', 'postinggoal_admin_update_plugin');
...
function postinggoal_install(){
    ...
    // Templates 
    postinggoal_templates();
    
    // STYLESHEET HINZUFÜGEN
	require_once MYBB_ADMIN_DIR."inc/functions_themes.php";
    // Funktion
    $css = postinggoal_stylesheet();
...
function postinggoal_admin_update_stylesheet(&$table) {
    ...
    // HINZUFÜGEN
    if ($mybb->input['action'] == 'add_master' AND $mybb->get_input('plugin') == "postinggoal") {
    ...
    $table->construct_cell("<b>".htmlspecialchars_uni("Inplaypostziel & Post-Challenges")."</b>", array('width' => '70%'));

    // Ob im Master Style vorhanden
    $master_check = $db->fetch_field($db->query("SELECT tid FROM ".TABLE_PREFIX."themestylesheets 
    WHERE name = 'postinggoal.css' 
    AND tid = 1
    "), "tid");
    ...
    if (!empty($masterstyle)) {
        ...
        $table->construct_cell("<a href=\"index.php?module=rpgstuff-stylesheet_updates&action=add_master&plugin=postinggoal\">".$lang->stylesheets_add."</a>", array('class' => 'align_center'));
    }
...
// Plugin Update
function postinggoal_admin_update_plugin(&$table) {
    ...
    // UPDATE
    if ($mybb->input['action'] == 'add_update' AND $mybb->get_input('plugin') == "postinggoal") {

        // Einstellungen überprüfen => Type = update
        postinggoal_settings('update');
        rebuild_settings();

        // Templates 
        postinggoal_templates('update');

        // Stylesheet
        $update_data = postinggoal_stylesheet_update();
        ...
        if (!empty($update_string)) {
            ...
            $masterstylesheet = $db->fetch_field($db->query("SELECT stylesheet FROM ".TABLE_PREFIX."themestylesheets WHERE tid = 1 AND name = 'postinggoal.css'"), "stylesheet");
            ...
            if ($pos === false) { // nicht vorhanden 
                ...
                while ($theme = $db->fetch_array($theme_query)) {
                    ...
                    $stylesheet_query = $db->simple_select("themestylesheets", "*", "name='".$db->escape_string('postinggoal.css')."' AND tid = ".$theme['tid']);
    ...
    $update_check = postinggoal_is_updated();

    if (!empty($update_check)) {
        $table->construct_cell($lang->plugins_actual, array('class' => 'align_center'));
    } else {
        $table->construct_cell("<a href=\"index.php?module=rpgstuff-plugin_updates&action=add_update&plugin=postinggoal\">".$lang->plugins_update."</a>", array('class' => 'align_center'));
    }
...
function postinggoal_global(){
    ...
    while ($post = $db->fetch_array($post_query)) {
        // Post
        $countposts++;
        $clean = postinggoal_count_words_characters($post['message']);
        // Wörter
        $countwords += $clean['words'];
        // Zeichen
        $countcharacters += $clean['characters'];
    }
    ...
    if($mybb->input['action'] == "postgoals_character"){
        ...
        while($topcharacterall = $db->fetch_array($topcharacterall_query)){
            ...
            while ($post = $db->fetch_array($topcharacter_query)) {
                ...
                $all_posts[$user] += 1;
                $clean = postinggoal_count_words_characters($post['message']);
                $all_words[$user] +=$clean['words'];
                $all_characters[$user] += $clean['characters'];
            }
        }
    ...
    if($mybb->input['action'] == "postgoals_player"){
        ...
        foreach ($all_mainaccounts as $mainuid) {
            ...
            while ($post = $db->fetch_array($topplayer_query)) {
                ...
                $all_posts[$mainuid] += 1;
                $clean = postinggoal_count_words_characters($post['message']);
                $all_words[$mainuid] += $clean['words'];
                $all_characters[$mainuid] += $clean['characters'];
            }
        }
    ...
    if($mybb->input['action'] == "postchallenges_overview_active"){
        ...
        while ($chell = $db->fetch_array($active_challenge)) {
            ...
            while ($post = $db->fetch_array($post_query)) {
                ...
                $clean = postinggoal_count_words_characters($post['message']);
                // Wörter
                $wordscount += $clean['words'];
                // Zeichen
                $characterscount += $clean['characters'];
            }
...
// MESSAGE BEREINIGEN
function postinggoal_clean_message($message) {
...
function postinggoal_count_words_characters($message) {

    global $mybb;

    $space_setting = $mybb->settings['postinggoal_space'];

    $messageClean = postinggoal_clean_message($message);
    ...
    if ($space_setting == 1) {
        ...
        // mit Leerzeichen
        $character_count = mb_strlen($messageClean);
    }
...
// EINSTELLUNGEN
function postinggoal_settings($type = 'install') {
    ...
    $setting_array = array(
		...
		'postinggoal_space' => array(
			'title' => 'Leerzeichen',
            'description' => 'Soll bei der Zeichenanzahl ohne Leerzeichen gezählt werden?',
            'optionscode' => 'yesno',
            'value' => '1', // Default
            'disporder' => 3
		),
    ...
    $gid = $db->fetch_field($db->write_query("SELECT gid FROM ".TABLE_PREFIX."settinggroups WHERE name = 'postinggoal' LIMIT 1;"), "gid");
...
// TEMPLATES
function postinggoal_templates($mode = '') {
...
// STYLESHEET MASTER
function postinggoal_stylesheet() {
...
// STYLESHEET UPDATE
function postinggoal_stylesheet_update() {
